...
- 新增「怪獸對戰模式」作為 Home.tsx 第三個 Tab，**取代原有「測試模式」**；保留「單題模式」與「10題模式」兩個 Tab 原有行為不變
...
- 10 隻怪獸依隨機順序出場，每次對戰只面對 1 隻
- 每隻怪獸初始 HP = 100%
- **每答對 1 題，當前怪獸扣血；答錯時怪獸不扣血**
- **怪獸切換觸發條件：當前怪獸 HP 歸零（≤ 0%）→ 播放擊敗動畫 → 切換下一隻**
- 若題目用完時怪獸仍未被擊敗，保持現狀直接進入結算（不強制切換）
- 若 10 隻怪獸全數擊敗，剩餘題目繼續作答（不再顯示怪獸），直至結算

**傷害量計算**：依題庫總題數動態計算每題傷害，確保恰好答對 `qPerMonster` 題即可擊敗一隻怪獸：

```text
qPerMonster    = ceil(totalQ / 10)          ← 擊敗一隻怪獸所需答對題數
damage         = floor(100 / qPerMonster)   ← 每答對 1 題的傷害量（%）
monsterHP      = 100 - (對當前怪獸答對題數 × damage)

範例（50 題）：qPerMonster = 5，damage = 20%，答對 5 題 → HP = 0 → 擊敗
```

> **重要**：`qPerMonster` 僅用於計算傷害量，**不做為怪獸切換的時間觸發點**。
> 怪獸切換 = HP 歸零事件，與題目索引無關。
...
```text
playerHP = ((maxWrong - wrongCount) / maxWrong) × 100
maxWrong = floor(totalQ × 0.3)   ← 動態計算，非固定 15 題
```
...
| 10  | 進入下一題 / 換怪 | 更新 currentIndex；若當前怪獸 HP = 0，播放擊敗動畫後切換 currentMonsterIndex +1 |
...
- [ ] 10 隻怪獸隨機排序出場；`qPerMonster = ceil(totalQ / 10)` 計算每題傷害量；**怪獸切換僅由 HP 歸零觸發**

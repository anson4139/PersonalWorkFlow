...
export async function onRequest(
  ...
  if (request.method === "GET") {
    ...
    const countParams: (string | number)[] = status ? [status] : [];
    const total = await env.BLOG_DB.prepare(
      `SELECT COUNT(*) as n FROM posts p ${where}`,
    )
      .bind(...countParams)
      .first<{ n: number }>();
  ...
  if (request.method === "POST") {
    ...
    const res = await env.BLOG_DB.prepare(
      `INSERT INTO posts (title, slug, content, excerpt, cover_url, status, published_at)
       VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id`,
    )
      .bind(
        body.title.trim(),

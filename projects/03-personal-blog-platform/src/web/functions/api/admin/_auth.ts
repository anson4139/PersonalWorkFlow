...
function hasPipelineSecret(
  request: { headers: { get(name: string): string | null } },
  env: Env,
): boolean {
  const auth = request.headers.get("Authorization");
  return Boolean(
    env.PIPELINE_SECRET && auth === `Bearer ${env.PIPELINE_SECRET}`,
  );
}
...
export async function requireAdmin(
  ...
  const email = await resolveEmail(request, env);
  if (!email) {
    if (hasPipelineSecret(request, env)) return null;
...
export function slugify(text: string): string {
  const ascii = text
    .toLowerCase()
    .trim()
    .replace(/[\u2E80-\u9FFF\uF900-\uFAFF\uFE30-\uFE4F\uFF00-\uFFEF]+/g, "-") // strip CJK
    .replace(/[\s\W-]+/g, "-")
    .replace(/^-+|-+$/g, "");
  // If the result is empty (e.g. pure CJK input), fall back to timestamp
  if (!ascii) {

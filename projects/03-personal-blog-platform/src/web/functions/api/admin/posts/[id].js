...
export async function onRequest(ctx) {
    ...
    if (request.method === "GET") {
        const post = await env.BLOG_DB.prepare(`SELECT p.*,
    ...
    if (request.method === "PUT") {
        ...
        const existing = await env.BLOG_DB.prepare(`SELECT id, title, slug, content, excerpt, cover_url, status, published_at FROM posts WHERE id = ?`)
            .bind(id)
            .first();
        if (!existing)
            return json({ error: "Not found" }, 404);
        ...
        const newPublishedAt = newStatus === "published"
            ...
            : existing.published_at;
        await env.BLOG_DB.prepare(`UPDATE posts SET title=?, slug=?, content=?, excerpt=?, cover_url=?, status=?, published_at=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`)
            .bind(newTitle, newSlug, body.content ?? existing.content, body.excerpt ?? existing.excerpt, body.cover_url !== undefined
            ? body.cover_url || null
            : existing.cover_url, newStatus, newPublishedAt, id)
            .run();
        // 重設分類
